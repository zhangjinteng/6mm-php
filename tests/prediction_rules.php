...
$rules = [
    ...
    ['game_type' => 'UP_DOWN', 'symbol' => 'RPCONLYUSDT', 'duration_seconds' => 30, 'enabled_by_default' => true],
];
$symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'btcusdt', ''];
$catalog = new PredictionRuleCatalog();

$all = $catalog->mergeAndFilter($rules, $symbols, PredictionGameType::UP_DOWN, PredictionRuleStatus::ALL);
...
$assertSame(
    ...
    array_map(static fn (array $rule): string => $rule['symbol'] . ':' . $rule['duration_seconds'], $all),
...
$highLow = $catalog->mergeAndFilter($rules, $symbols, PredictionGameType::HIGH_LOW, PredictionRuleStatus::ALL);
